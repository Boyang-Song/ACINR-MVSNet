...
parser.add_argument('--max_h', type=int, default=960, help='Maximum image height')
parser.add_argument('--max_w', type=int, default=1280, help='Maximum image width')
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    ...
    mask2 = np.logical_and(depth_ref > 425, depth_ref < 935)
...
def filter_depth(cam_folder, scan_folder, out_folder, plyfilename, max_h, max_w, base_image_size):

    # the pair file
    pair_file = os.path.join(cam_folder, "pair.txt")
    ...
    for ref_view, src_views in pair_data:
        ...
        ref_intrinsics, ref_extrinsics = read_camera_parameters(
            os.path.join(cam_folder, '{:0>8}_cam.txt'.format(ref_view)))
        # load the reference image
        #ref_img = read_img(os.path.join(results_folder, '{:08d}.jpg'.format(ref_view)))
        ref_img = read_img(os.path.join(scan_folder, 'rect_{:0>3}_3_r5000.png'.format(ref_view+1)))

        h_scale = float(max_h) / ref_img.shape[0]
        w_scale = float(max_w) / ref_img.shape[1]
        ...
        ref_img = scale_image(ref_img, scale=resize_scale)
        ref_intrinsics = scale_camera(ref_intrinsics, scale=resize_scale)
        ref_img_h, ref_img_w = ref_img.shape[0:2]
        new_h = ref_img_h
        new_w = ref_img_w
        if new_h > max_h:
            new_h = max_h
        else:
            new_h = int(math.floor(ref_img_h / base_image_size) * base_image_size)
        if new_w > max_w:
            new_w = max_w
        else:
            new_w = int(math.floor(ref_img_w / base_image_size) * base_image_size)
        start_h = int(math.floor((ref_img_h - new_h) / 2))
        start_w = int(math.floor((ref_img_w - new_w) / 2))
        finish_h = start_h + new_h
        finish_w = start_w + new_w
        ref_img = ref_img[start_h:finish_h, start_w:finish_w]
        ref_intrinsics[0][2] = ref_intrinsics[0][2] - start_w
        ref_intrinsics[1][2] = ref_intrinsics[1][2] - start_h
        ...
        for src_view in src_views:
            ...
            src_intrinsics, src_extrinsics = read_camera_parameters(
                os.path.join(cam_folder, '{:0>8}_cam.txt'.format(src_view)))
            src_intrinsics = scale_camera(src_intrinsics, scale=resize_scale)
            src_intrinsics[0][2] = src_intrinsics[0][2] - start_w
            src_intrinsics[1][2] = src_intrinsics[1][2] - start_h
...
if __name__ == '__main__':
    ...
    img_folder = os.path.join(args.testpath, 'Rectified')
    cam_folder = os.path.join(args.testpath, 'Cameras')
    for scan in scans:
        scan_folder = os.path.join(img_folder, scan)
        out_folder = os.path.join(args.outdir, scan)
        ply_folder = os.path.join(args.outdir, 'ply')
        if not os.path.exists(ply_folder):
            print('ply_folder', ply_folder)
        ...
        scene_id = int(scan[4:])
        filter_depth(cam_folder, scan_folder, out_folder, os.path.join(ply_folder, 'acinrmvsnet{:0>3}_l3.ply'.format(scene_id)), max_h, max_w, base_image_size)